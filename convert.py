...
def parse_arg():
    """解析命令行的输入的参数"""
    parser = argparse.ArgumentParser("Python批量转换 视频 为 音频MP3（即提取音频文件）")
    parser.add_argument("file_path", help="输入文件、目录路径，如果为目录，则遍历目录下的文件")
    parser.add_argument("--output-dir", help="(可选)输出目录路径，如果不传，则使用输入文件目录")
    parser.add_argument("--traverse", action='store_true',
                        help="(可选)src-path为目录是，是否遍历子目录，默认False")
...
def main():
    ...
    print("转换的成功文件数：{}个".format(success_num))
    print("转换的失败文件数：{}个".format(fail_num))
    print("总耗时:{}秒".format(cost_seconds))
